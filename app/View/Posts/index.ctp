...
<h2>カテゴリ一覧</h2>
<?php echo $this->Html->link('Add category', array("controller" => "categories", "action" => "index")); ?>
...
<ul>
  <?php foreach ($categories as $category) : ?>
    ...
    */
      // echo $this->Html->link($post['Post']['title'], '/posts/view/' . $post['Post']['id']); // cakephpフォルダからの相対パスでのリンク記述
      // echo $this->Html->link($post['Post']['title'], 'http://localhost/blog/posts/view/' . $post['Post']['id']); // 絶対パスでリンク先を記述
      echo $this->Html->link($category['Category']['name'], array("controller" => "categories", "action" => 'view',  $category['Category']['id'])); // 配列でリンク先を記述

      echo $this->Html->link('編集', array('action' => 'edit', $category['Category']['id'])); // 編集用リンクを追加
      // echo $this->Form->postLink(
      //   '削除',
      //   array('action' => 'delete', $post['Post']['id']),
      //   array('confirm' => 'sure?')
